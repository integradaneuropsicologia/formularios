...
if (!window.ESAVIAData || !window.ESAVIAAccess || !window.ESAVIAScoring) {
  throw new Error("Não foi possível carregar os módulos da EsAvI-A.");
}

const data = window.ESAVIAData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
function renderDemoResults(scored) {
  const list = $("#demoFactorList");
  list.replaceChildren();

  scored.factors.forEach((factor) => {
    const card = document.createElement("article");
    card.className = "demo-factor";

    const title = document.createElement("h3");
    title.textContent = `Fator ${factor.code.slice(1)} - ${factor.name}`;

    const metrics = document.createElement("div");
    metrics.className = "demo-factor__metrics";
    [
      `${factor.rawScore} pontos brutos`,
      factor.percentile,
    ...
    interpretation.textContent = factor.interpretation;
    card.append(title, metrics, interpretation);
    list.append(card);
  });
...
function renderSuccess(scored) {
  ...
  if (state.demo) {
    ...
    $("#successStatus").textContent = "Confira abaixo os quatro resultados que serão enviados em results_meta.";
    $("#successBackButton").textContent = "Preencher novamente";
    renderDemoResults(scored);
    return;
  }
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__ESAVI_A_DEMO_SUBMISSION__ = {
        results,
