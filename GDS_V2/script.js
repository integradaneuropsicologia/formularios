...
if (!window.GDSData || !window.GDSAccess || !window.GDSScoring) {
  throw new Error("Não foi possível carregar os módulos da GDS-15.");
}

const data = window.GDSData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__GDS_DEMO_SUBMISSION__ = {
        results,
