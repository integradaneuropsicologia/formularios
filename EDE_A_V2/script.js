...
if (!window.EDEAData || !window.EDEAAccess || !window.EDEAScoring) {
  throw new Error("Não foi possível carregar os módulos da EDE-A.");
}

const data = window.EDEAData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__EDE_A_DEMO_SUBMISSION__ = {
        results,
