...
if (!window.PSSData || !window.PSSAccess || !window.PSSScoring) {
  throw new Error("Não foi possível carregar os módulos da PSS-10.");
}

const data = window.PSSData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__PSS_DEMO_SUBMISSION__ = {
        results,
