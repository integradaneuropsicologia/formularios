...
test("valida token e código fixo do formulário", () => {
  assert.deepEqual(
    access.validateRequest("?token=abc&form=PSS_V2"),
    { token: "abc", formCode: "PSS_V2" }
  );
  assert.throws(() => access.validateRequest("?form=PSS_V2"), /Link inválido/);
  assert.throws(
...
test("consulta a liberação pública pelo RPC esperado", async () => {
  ...
  const result = await access.fetchPatientAccess(
    client,
    "?token=token-valido&form=PSS_V2"
  );
...
test("envia o payload estrito pelo RPC público", async () => {
  ...
  await access.submitPatientResponse(client, {
    search: "?token=token-valido&form=PSS_V2",
    results: [{
      pergunta: "No mês passado, com que frequência você ficou chateado(a) por causa de algo que aconteceu inesperadamente?",
      resposta: "Às vezes"
    }],
    resultsMeta: {
      pontuacao_bruta_total: 2
    }
  });

  assert.equal(calls[0].name, "submit_public_patient_form_response");
  assert.equal(calls[0].payload.p_form_code, "PSS_V2");
  assert.equal(calls[0].payload.p_form_name, "PSS-10 - Escala de Estresse Percebido");
...
test("retorna à área do paciente preservando o token", () => {
  const url = new URL(access.buildPatientAreaUrl("?token=abc&form=PSS_V2"));
