(function initializePssScoring(globalScope) {
  "use strict";

  const EXPECTED_REVERSED_ITEMS = [4, 5, 7, 8];
  ...
  function validateData(data) {
    if (!data || !Array.isArray(data.questions) || data.questions.length !== 10) {
      throw new Error("A PSS-10 deve conter exatamente 10 itens.");
    }

    if (!Array.isArray(data.responses) || data.responses.length !== 5) {
      throw new Error("A PSS-10 deve conter exatamente 5 alternativas.");
    }
    ...
    if (
      ...
      throw new Error("As alternativas da PSS-10 devem representar internamente os valores de 0 a 4.");
    }
    ...
    if (JSON.stringify(reversedItems) !== JSON.stringify(EXPECTED_REVERSED_ITEMS)) {
      throw new Error("A lista de itens invertidos da PSS-10 está incorreta.");
    }
  }
