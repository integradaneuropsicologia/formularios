...
if (!window.GAIData || !window.GAIAccess || !window.GAIScoring) {
  throw new Error("Não foi possível carregar os módulos do GAI.");
}

const data = window.GAIData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__GAI_DEMO_SUBMISSION__ = {
        results,
