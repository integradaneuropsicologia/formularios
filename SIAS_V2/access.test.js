...
test("valida token e código fixo do formulário", () => {
  assert.deepEqual(
    access.validateRequest("?token=abc&form=SIAS_V2"),
    { token: "abc", formCode: "SIAS_V2" }
  );
  assert.throws(() => access.validateRequest("?form=SIAS_V2"), /Link inválido/);
  assert.throws(
...
test("consulta a liberação pública pelo RPC esperado", async () => {
  ...
  const result = await access.fetchPatientAccess(
    client,
    "?token=token-valido&form=SIAS_V2"
  );
...
test("envia o payload estrito pelo RPC público", async () => {
  ...
  await access.submitPatientResponse(client, {
    search: "?token=token-valido&form=SIAS_V2",
    results: [{
      pergunta: "Fico nervoso(a) se tiver que falar com alguém que tem autoridade.",
      resposta: "Muito característico de mim"
    }],
    resultsMeta: {
      pontuacao_bruta_total: 3
    }
  });

  assert.equal(calls[0].name, "submit_public_patient_form_response");
  assert.equal(calls[0].payload.p_form_code, "SIAS_V2");
  assert.equal(
    calls[0].payload.p_form_name,
    "SIAS - Escala de Ansiedade na Interação Social"
  );
...
test("retorna à área do paciente preservando o token", () => {
  const url = new URL(access.buildPatientAreaUrl("?token=abc&form=SIAS_V2"));
