...
if (!window.SIASData || !window.SIASAccess || !window.SIASScoring) {
  throw new Error("Não foi possível carregar os módulos do SIAS.");
}

const data = window.SIASData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
function renderQuestions() {
  ...
  data.questions.forEach((question, index) => {
    ...
    getResponseOptions(data).forEach((option) => {
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__SIAS_DEMO_SUBMISSION__ = {
        results,
