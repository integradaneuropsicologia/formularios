(function initializeProteaAccess(globalScope) {
  "use strict";

  const FORM_CODE = "PROTEA_R_NV_V2";
  const FORM_NAME = "PROTEA-R-NV - Protocolo de Avaliação Comportamental";
  ...
  async function submitPatientResponse(client, { search, results, resultsMeta }) {
    ...
    if (!submission?.ok) {
      throw new Error("O banco não confirmou o salvamento da avaliação.");
    }
