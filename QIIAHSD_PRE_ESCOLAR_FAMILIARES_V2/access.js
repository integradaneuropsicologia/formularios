(function initializeQiiAhsdPreEscolarFamiliaresAccess(globalScope) {
  "use strict";

  const FORM_CODE = "QIIAHSD_PRE_ESCOLAR_FAMILIARES_V2";
  const FORM_NAME = "Questionário Complementar de Indicadores de Altas Habilidades/Superdotação - Pré-escolar - Familiares";
  ...
  function getFormCode(search) {
    const params = getSearchParams(search);
    return String(params.get("form") || params.get("code") || FORM_CODE)
      .trim()
      .toUpperCase();
  }
  ...
  async function fetchPatientAccess(client, search) {
    ...
    if (error) throw new Error(mapAccessError(error));

    const access = firstRow(data);
    if (!access) throw new Error("Paciente não encontrado para este link.");
  ...
  async function submitPatientResponse(client, { search, results, resultsMeta }) {
    ...
    if (error) throw new Error(mapSubmissionError(error));
  ...
  if (globalScope) {
    globalScope.QIIAHSDPreEscolarFamiliaresAccess = api;
  }
