...
if (!window.YBOCSData || !window.YBOCSAccess || !window.YBOCSScoring) {
  throw new Error("Não foi possível carregar os módulos da Y-BOCS.");
}

const data = window.YBOCSData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
function renderQuestions() {
  ...
  data.questions.forEach((question, index) => {
    if (question.section !== activeSection) {
      ...
      const section = data.sections.find((item) => item.id === activeSection);
    ...
    itemTitle.className = "question-title";
    itemTitle.textContent = question.title;

    legend.append(itemNumber, itemTitle);
    ...
    fieldset.append(legend, prompt, options);
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__YBOCS_DEMO_SUBMISSION__ = {
        results,
