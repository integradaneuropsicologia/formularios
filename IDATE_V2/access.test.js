...
test("valida token e código fixo do formulário", () => {
  assert.deepEqual(
    access.validateRequest("?token=abc&form=IDATE_V2"),
    { token: "abc", formCode: "IDATE_V2" }
  );
  assert.throws(() => access.validateRequest("?form=IDATE_V2"), /Link inválido/);
  assert.throws(
...
test("consulta a liberação pública pelo RPC esperado", async () => {
  ...
  const result = await access.fetchPatientAccess(
    client,
    "?token=token-valido&form=IDATE_V2"
  );
...
test("envia o payload estrito pelo RPC público", async () => {
  ...
  await access.submitPatientResponse(client, {
    search: "?token=token-valido&form=IDATE_V2",
    results: [{
      pergunta: "Sinto-me bem",
  ...
  assert.equal(calls[0].payload.p_form_code, "IDATE_V2");
  assert.equal(calls[0].payload.p_form_name, "IDATE - Inventário de Ansiedade Traço");
...
test("retorna à área do paciente preservando o token", () => {
  const url = new URL(access.buildPatientAreaUrl("?token=abc&form=IDATE_V2"));
