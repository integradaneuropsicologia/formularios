...
if (!window.IDATEData || !window.IDATEAccess || !window.IDATEScoring) {
  throw new Error("Não foi possível carregar os módulos do IDATE.");
}

const data = window.IDATEData;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__IDATE_DEMO_SUBMISSION__ = {
        results,
