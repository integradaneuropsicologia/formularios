(function initializeTolAccess(globalScope) {
  "use strict";

  const FORM_CODE = "TORRE_DE_LONDRES_V2";
  const FORM_NAME = "Torre de Londres (TOL-BR)";
  ...
  function validateRequest(search) {
    ...
    if (!token) {
      throw new Error("Link inválido ou expirado. Abra o teste pela Área do Paciente.");
    }
  ...
  function mapAccessError(error) {
    ...
    if (message.includes("invalido") || message.includes("expirado")) {
      return "Link inválido ou expirado. Abra o teste novamente pela Área do Paciente.";
    }
  ...
  function mapSubmissionError(error) {
    ...
    if (message.includes("ja preenchido")) {
    ...
    return "Não foi possível salvar o resultado. Tente novamente.";
  }
  ...
  async function submitPatientResponse(client, { search, results, resultsMeta }) {
    ...
    if (!submission?.ok) {
      throw new Error("O banco não confirmou o salvamento do resultado.");
    }
