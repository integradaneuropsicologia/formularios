...
if (!window.GAD7Data || !window.GAD7Access || !window.GAD7Scoring) {
  throw new Error("Não foi possível carregar os módulos do GAD-7.");
}

const data = window.GAD7Data;
const {
  buildPatientAreaUrl,
  fetchPatientAccess,
...
async function handleSubmit(event) {
  ...
  try {
    if (state.demo) {
      window.__GAD7_DEMO_SUBMISSION__ = {
        results,
