...
test("valida token e código fixo da Forma B", () => {
  assert.deepEqual(
    access.validateRequest("?token=abc&form=ESAVI_B_V2"),
    { token: "abc", formCode: "ESAVI_B_V2" }
  );
  assert.throws(() => access.validateRequest("?form=ESAVI_B_V2"), /Link inválido/);
  assert.throws(
    () => access.validateRequest("?token=abc&form=ESAVI_A_V2"),
    /outro formulário/
  );
});

test("consulta a liberação pública da Forma B", async () => {
  const calls = [];
  const client = {
  ...
  const result = await access.fetchPatientAccess(
    client,
    "?token=token-valido&form=ESAVI_B_V2"
  );
  ...
  assert.deepEqual(calls, [{
    ...
    payload: {
      ...
      p_form_code: "ESAVI_B_V2"
    }
  }]);
});

test("envia results e results_meta pelo RPC público", async () => {
  const calls = [];
  const client = {
    async rpc(name, payload) {
      calls.push({ name, payload });
  ...
  await access.submitPatientResponse(client, {
    search: "?token=token-valido&form=ESAVI_B_V2",
    results,
    resultsMeta
  });

  assert.equal(calls[0].name, "submit_public_patient_form_response");
  assert.equal(calls[0].payload.p_form_code, "ESAVI_B_V2");
  assert.equal(
    calls[0].payload.p_form_name,
    "EsAvI-B - Escala de Avaliação da Impulsividade"
  );
...
test("retorna à área do paciente preservando o token", () => {
  const url = new URL(access.buildPatientAreaUrl("?token=abc&form=ESAVI_B_V2"));
